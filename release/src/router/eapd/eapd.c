 ...
 *
 * $Id: eapd.c 407896 2013-06-14 23:31:52Z $
 */
...
eapd_wksp_find_cb(eapd_app_t *app, char *wlifname, uint8 *mac)
{
	...
	if (!cb)
		EAPD_ERROR("No cb found for interface %s\n", ifname);
	...
	if (status > 0) {
		...
		if (nas->appSocket >= 0 &&
			...
			if (bytes > ETHER_HDR_LEN) {
				...
				eth = (struct ether_header*)(ifname + IFNAMSIZ);
				bytes -= IFNAMSIZ;
				EAPD_INFO("ifname %s to find cb for mac %s", ifname, ether_etoa((uchar *)eth->ether_shost, eabuf));

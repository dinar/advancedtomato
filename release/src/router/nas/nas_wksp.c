		...
		if (nwksp->eapd !=  NAS_WKSP_UNK_FILE_DESC && FD_ISSET(nwksp->eapd, &fdset)) {
			...
			if ((bytes = recv(nwksp->eapd, pkt, len, 0)) > 0) {
				...
				case ETHER_TYPE_802_1X: /* eapol */
					NASDBG("recv eapol packet from socket %d ifname %s type %d\n",
					       nwksp->eapd, ifname, eapol->type);
				...
				case ETHER_TYPE_802_1X_PREAUTH: /* preauth */
					NASDBG("recv preauth packet from socket %d type %d\n",
					       nwksp->eapd, eapol->type);
				...
				case ETHER_TYPE_BRCM: /* brcmevent */
					...
					NASDBG("%s: %02x:%02x:%02x:%02x:%02x:%02x event_type %u\n",
						...
						(uint8)pvt_data->event.event_type);
